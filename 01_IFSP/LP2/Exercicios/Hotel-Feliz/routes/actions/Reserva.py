...
def criar_reserva():
    ...
    if not (id_hospede and checkin and checkout):
        return jsonify({"error": "id_hospede_principal, data_checkin e data_checkout obrigatÃ³rios"}), 400
    try:
    ...
    if not hosp:
        return jsonify({"error": "HÃ³spede nÃ£o encontrado"}), 404
    valor_total = Decimal("0.00")
    if numero_quarto:
        quarto = Quarto.query.get(numero_quarto)
        if not quarto:
            return jsonify({"error": "Quarto nÃ£o encontrado"}), 404
...
def listar_reservas():
    res = paginate_or_all(Reserva.query)
...
def atualizar_reserva(id_reserva):
    ...
    if "numero_quarto" in data:
        if data["numero_quarto"] is not None:
            ...
            if not q:
                return jsonify({"error": "Quarto invÃ¡lido"}), 400
