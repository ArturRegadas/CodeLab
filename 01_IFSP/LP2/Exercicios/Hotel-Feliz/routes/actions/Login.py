...
def register():
    ...
    if not nome or not email or not senha or not documento:
        return jsonify({"error": "nome_completo, email, senha e documento são obrigatórios"}), 400

    if Usuario.query.filter_by(email=email).first():
        return jsonify({"error": "Email já cadastrado"}), 400

    perfil = Perfil.query.filter_by(nome_perfil="Hóspede").first()
    if not perfil:
        perfil = Perfil(nome_perfil="Hóspede")
    ...
    return jsonify({"message": "Usuário e hóspede criados com sucesso!"}), 201